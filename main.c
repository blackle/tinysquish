...
static void memory_writer_new(WriteInterface* writer, size_t size)
{
...
static bool memory_writer_compare(const WriteInterface* writer, const uint8_t* data, size_t size)
{
	const MemoryWriter* mw = (MemoryWriter*) writer->private_data;
	if (size != mw->position) {
		return false;
	}
...
static size_t memory_writer_length(const WriteInterface* writer)
{
	const MemoryWriter* mw = (MemoryWriter*) writer->private_data;
	return mw->position;
}
...
static void memory_reader_new(ReadInterface* reader, const uint8_t* data, size_t size)
{
...
static void memory_reader_from_writer(ReadInterface* reader, const WriteInterface* writer)
{
	MemoryWriter* mw = (MemoryWriter*) writer->private_data;
	memory_reader_new(reader, mw->data, mw->position);
}
...
int main(int argc, char** argv)
{
	...
	WriteInterface memory_writer_compressed;
	memory_writer_new(&memory_writer_compressed, 10000);
	...
	ReadInterface memory_reader;
	memory_reader_from_writer(&memory_reader, &memory_writer_compressed);

	WriteInterface memory_writer_decompressed;
	memory_writer_new(&memory_writer_decompressed, data_len);
